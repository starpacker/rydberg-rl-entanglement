...
class NoiseModel:
    ...
    def sample(self, rng: np.random.Generator) -> Dict[str, Any]:
        ...
        if "doppler" in self.active:
            result["delta_doppler"] = [
                float(rng.normal(0, SIGMA_DOPPLER)) for _ in range(self.n_atoms)
            ]
        else:
        ...
        if "position" in self.active:
            result["delta_R"] = [
                float(rng.normal(0, SIGMA_POSITION)) for _ in range(self.n_atoms)
            ]
        else:
        ...
        if "amplitude" in self.active:
            result["ou_sigma"] = OU_SIGMA
            result["ou_tau"] = OU_CORRELATION_TIME
        else:
            result["ou_sigma"] = 0.0
            result["ou_tau"] = OU_CORRELATION_TIME  # irrelevant if sigma=0

        # 4. Phase noise (static Gaussian)
        if "phase" in self.active:
            # Laser linewidth contributes a random phase offset each shot
            # Typical laser linewidth ~ 1 kHz => sigma_phase ~ 2pi*1e3 * T_gate
            # We use a moderate value; the RL agent should learn to be robust
            sigma_phase = 2 * np.pi * 1e3 * self.cfg["T_gate"]
    ...
    def generate_ou_series(
        ...
        """
        sigma = OU_SIGMA if "amplitude" in self.active else 0.0
