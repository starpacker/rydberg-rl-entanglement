"""Real Noise-Conditioned Open-Loop PPO Training.

This is ACTUAL PPO (not BC) for noise-conditioned open-loop control.

Key techniques to make PPO work in high-dimensional noise space:
1. BC warm-start (PPOfD): Initialize policy from BC demonstrations
2. Curriculum learning: Start with low noise, gradually increase
3. Better reward shaping: Dense rewards throughout episode
4. Fourier action space: Policy outputs Fourier params, not raw actions

The policy architecture:
    obs = [t/T, noise_params_normalized] (7-dim)
    policy outputs: Fourier_correction (20-dim) at t=0, cached for episode
...
class FourierOpenLoopEnv(gym.Env):
    """Single-step environment for noise-conditioned open-loop control.

    This is a bandit/one-step MDP:
    - Observation: [noise_params_normalized] (6-dim)
    ...
    This is much more efficient for PPO than multi-step with dummy actions.
    """
    ...
    def __init__(
        ...
        self.n_fourier = 5
        t = np.linspace(0, 1, n_steps, endpoint=False)  # match CMA-ES convention
        ...
        # Observation: noise_params(6) - no time needed for single-step
        ...
        self._noise_vec: Optional[np.ndarray] = None
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Get noise vector (6-dim)
        self._noise_vec = self._env._get_noise_vector_normalized()

        return self._noise_vec.copy(), {}
    ...
    def step(self, action: np.ndarray):
        ...
        return self._noise_vec.copy(), fidelity, True, False, {"fidelity": fidelity}
...
def train_ppo_openloop(
    ...
    for seed_idx in range(n_seeds):
        ...
        print(f"\n{'='*70}")
        print(f"REAL Noise-Conditioned Open-Loop PPO, seed={seed}")
        print(f"  {total_timesteps/1e6:.1f}M steps, Fourier action space (20-dim)")
        ...
        # Save model
        model_path = models_dir / f"ppo_openloop_real_seed{seed}"
    ...
    if best_model is not None:
        best_path = models_dir / "ppo_openloop_real_best"
        best_model.save(str(best_path))
        print(f"\nBest model: F = {best_fid:.4f}, saved to {best_path}")

    # Save logs
    log_path = results_dir / "ppo_openloop_real_training.json"
...
def evaluate_ppo_openloop(
    ...
    if model is None:
        model_path = ROOT / "models" / "ppo_openloop_real_best"
        model = PPO.load(str(model_path))
        print(f"Loaded model from {model_path}")

    print(f"\n{'='*60}")
    print("Evaluate REAL Open-Loop PPO")
    print("="*60)
...
def main():
    ...
    if args.eval or not args.train:
        ...
        out_path = ROOT / "results" / "ppo_openloop_real_eval.json"
        ...
        # Print comparison
        print("\n" + "="*70)
        print("COMPARISON: Real PPO Open-Loop vs Baselines")
        print("="*70)
